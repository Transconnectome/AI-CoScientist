# K-NeuroMind 제안서 작성 전략

**분석일**: 2025-10-18
**양식**: 2026년도 인공지능 분야 신규 R&D 사업 사업보완 제출 양식
**작성 방향**: 원본 grant.hwpx 형식 준수 + Enhanced Proposal 내용 활용
...
```
1. 사업 내용 (테이블 형식, 10개 필드)
2. 인공지능 R&D 특성 (3개 하위섹션, 다중 테이블)
3. 주요 기술개발 사항 (연도별 상세 계획)
4. 예산 규모 (5개년 세부 항목별)
```
...
#### 1.2 사업 목적 (**개선 필요**)
**현재 문제**:
- 너무 기술적, 일반인이 이해하기 어려움
- "개인의 인지 및 감정 상태 해독" → 모호함

**개선 전략**:
```
【Crisis Framing + Solution + Impact】

"한국은 세계 최고령 사회로 진입하며, 2030년까지 치매 환자가 180만 명으로 2배 증가할 것으로 예상됩니다(보건복지부, 2024). 현재 치매는 증상 발현 후 진단되어 뇌손상이 이미 진행된 상태로 발견되며, 이로 인해 연간 ₩20.7조의 의료비가 소요됩니다.

본 사업은 이 문제를 해결하기 위해 fMRI, dMRI, EEG 등 다중 모달리티 뇌 데이터를 AI로 통합 분석하여, '증상 발현 3-5년 전' 질병을 예측하는 '한국형 브레인 파운데이션 모델'을 개발합니다. 이를 통해 조기 개입으로 뇌질환 진행을 지연시키고, 국민 건강 수명을 연장하며, 의료비를 연간 ₩5조 절감하는 것을 목표로 합니다."
```

**작성 원칙**:
1. 첫 문장: 위기 설정 (치매 증가, 의료비 폭등)
2. 중간: 기술적 솔루션 (다중 모달리티 AI)
3. 마지막: 정량적 임팩트 (의료비 절감)

#### 1.3 사업 주요내용 (**대폭 개선 필요**)
**현재 문제**:
- 1단계/2단계 구분만 있고 세부사항 부족
- "개별 양식 모델" 같은 모호한 용어
- 정량적 목표 전무

**개선 전략**: Enhanced Proposal의 기술 내용을 **압축**하여 표 형식에 맞게 재구성
...
1) 다중 모달리티 데이터 통합 인프라 구축
   ...
   - 품질 관리: 자동화 QA 파이프라인 (motion artifacts 제거, SNR 검증)

2) 모달리티별 인코더 개발
   - fMRI 인코더: Vision Transformer (ViT-L/16), 304M 파라미터
   - dMRI 인코더: Graph Attention Network (GAT), 45M 파라미터
   - EEG 인코더: 1D CNN, 18M 파라미터
   - 각 모달리티별 Validation Accuracy > 80% 달성
...
4) 개념 검증 (Proof-of-Concept)
   - 알츠하이머 예측: AUC > 0.85 (vs. 기존 SOTA 0.68)
   - 3개 국제 벤치마크 테스트: ADNI, ABIDE, fMRI-IQ
...
1) 파운데이션 모델 성능 향상
   ...
   - 실시간 BCI 프로토타입: 인지 상태 디코딩 < 200ms latency

2) 임상 검증 (Clinical Trial)
   - 병원: 5개 (삼성의료원, 서울대병원, 아산병원, 세브란스, 분당서울대병원)
   - 규모: 전향적 RCT 1,000명
   - 1차 종료점: 진단 시간 30% 단축
   - 2차 종료점: 의료비 20% 절감

3) K-NeuroMind Open Platform 구축
   - 공개 모델: M³-MAE 사전학습 가중치 (456M params)
   - 공개 데이터: 5,000명 익명화 뇌 스캔 (BIDS 포맷)
   - 컴퓨팅 자원: KISTI 클라우드 무료 100,000 GPU-hours/년
   - 목표 사용자: 국내외 연구자 1,000+ 명
```

**핵심 개선사항**:
- ✅ 정량적 목표 모두 명시 (데이터 규모, 모델 크기, 성능 지표)
- ✅ 기술 용어 구체화 ("개별 양식 모델" → "Vision Transformer")
- ✅ 검증 계획 명확화 (국제 벤치마크, 임상시험)
- ✅ 오픈 플랫폼 구체화 (공개 범위, 사용자 목표)
...
#### 1.7 AI R&D 비중 (그대로)
```
총 553.23억 원 (인공지능바이오) 중 16억 원 (2.89%)
```
...
**개선 전략**:
```
【Why Government? Why Now? Why Korea?】

1. 정부 지원의 필연성
   - 뇌 데이터 수집 비용: 1인당 ₩1M (fMRI+dMRI+EEG) × 10,000명 = ₩100억
   - GPU 컴퓨팅 비용: 64 H100 × 3개월 = ₩8억
   - 임상시험 비용: 1,000명 RCT = ₩10억
   → 총 초기 투자 ₩118억은 민간 단독 감당 불가, 정부 주도 필수

2. 시급성 (Why Now?)
   - 한국 고령화 속도: 세계 1위 (2030년 초고령사회 진입)
   - 치매 환자: 2020년 84만 → 2030년 180만 (2.14배 급증)
   - 의료비: 현재 ₩20.7조/년 → 2030년 ₩40조/년 (WHO 예측)
   → 지금 투자하지 않으면 향후 10년간 ₩200조 이상 의료비 폭증 불가피

3. 한국의 경쟁우위 (Why Korea?)
   - 데이터: 15년 축적된 KBIS 뇌 데이터 5,000명 (세계 3위 규모)
   - 인프라: KISTI 슈퍼컴퓨터 Nurion (25.7 petaflops), NVIDIA AI Hub 파트너십
   - 제도: 전국민 건강보험 → 대규모 임상 데이터 연계 가능
   - 인력: 삼성/LG/NAVER AI 인재 + 서울대/KAIST 뇌과학 연구자 결합
   → 국제 경쟁(NIH BRAIN Initiative $6.6B, EU HBP €607M) 대비 효율적 투자로 차별화 가능

4. 기대 효과
   - 과학: Nature/Science급 논문 10+편, 한국을 AI-뇌과학 글로벌 허브로 도약
   - 경제: ₩1.2조 브레인 헬스 AI 산업 창출, 500+ 고급 일자리
   - 의료: 조기 진단으로 치매 진행 지연 → 연간 ₩5조 의료비 절감
   - 수출: K-NeuroMind 모델 라이선스 → 일본/싱가포르/중국 의료기관 수출 (₩20억/년)
```

**작성 원칙**:
1. 왜 정부가 해야 하는가? → 민간 불가능한 초기 투자 규모
2. 왜 지금인가? → 치매 급증 위기
3. 왜 한국인가? → 데이터/인프라/인력 경쟁우위
4. 무엇을 얻는가? → 과학/경제/의료/수출 4가지 측면

#### 1.10 AI 개발 관련 선행사업 (그대로)
```
선행사업 없음
```
...
**테이블 1: 데이터 수집 방법 (퍼센티지 체크)**

| 데이터 확보 방안 | 비율 | 상세 설명 |
|----------------|------|----------|
| 데이터 신규 구축 | 20% | 2,000명 신규 스캔 (fMRI+dMRI+EEG), 2026-2028 수집, ₩20억 |
| 기존 데이터 확장 | 10% | KBIS 데이터 추가 스캔 (1,000명), 종단 추적 강화 |
| 기 구축된 타기관 데이터 활용 | 50% | KBIS 5,000명 + 치매센터 3,000명, 데이터 사용 협약 체결 |
| 기 구축된 자체 데이터 활용 | 20% | 연구팀 보유 병원 데이터 2,000명, 재가공 후 활용 |
...
**테이블 2: 데이터 전처리 (퍼센티지 체크)**

| 학습·평가 데이터셋 준비 | 비율 | 상세 설명 |
|---------------------|------|----------|
| 데이터 기초 전처리 | 25% | • Motion correction (MCFLIRT)<br>• Skull stripping (BET)<br>• Outlier detection (Mahalanobis distance > 3σ 제거)<br>• Missing value imputation (KNN, k=5) |
| 데이터 증강 및 변환 적용 | 25% | • fMRI: Temporal jittering (±2 TR)<br>• dMRI: Fiber tracking augmentation (probabilistic tractography 5회 반복)<br>• EEG: 시간축 stretch/compress (0.9-1.1x)<br>• 정규화: Z-score normalization 전역 적용 |
| 대량데이터 자동처리 및 최적화 | 25% | • Freesurfer recon-all 자동화 (SGE cluster 병렬처리)<br>• DICOM → NIfTI 변환 자동화 (dcm2niix)<br>• 데이터 라벨링 준자동화 (Active Learning, BALD sampling)<br>• HDF5 포맷 최적화 (chunk size 64MB, compression gzip level 4) |
| 실시간 데이터 품질 관리 및 지속적인 갱신 | 25% | • 실시간 QA 대시보드 (Grafana + Prometheus)<br>• Drift detection (Kolmogorov-Smirnov test, p<0.05 경고)<br>• 자동 이상치 탐지 (Isolation Forest, contamination=0.05)<br>• 분기별 데이터 재검증 (inter-rater reliability κ > 0.80 유지) |
...
- fMRI augmentation: Random cropping (3D patches), temporal jittering
...
**테이블 4: AI 모델 개발 수준 (복수 선택 가능)**

| 개발 수준 | 비율 | 상세 설명 |
|----------|------|----------|
| 오픈모델 활용 | 50% | • ViT-L/16 (ImageNet 사전학습) → fMRI 도메인 fine-tuning<br>• GAT (ogbn-arxiv 사전학습) → brain connectome fine-tuning<br>• Hugging Face Transformers 라이브러리 활용 |
| 신규모델 개발 | 15% | • Multi-Modal Masked Autoencoder (M³-MAE) 아키텍처 설계<br>• 뇌 데이터 특화 positional encoding (3D + temporal)<br>• Cross-modal attention mechanism 설계 |
| 새로운 알고리즘 개발 | 15% | • Masked multi-modal reconstruction loss 함수 개발<br>• Cross-modal contrastive learning (InfoNCE 변형)<br>• Brain-specific augmentation 전략 (temporal jittering, spatial cropping) |
| 민간 모델 도입/응용 | 10% | • OpenAI CLIP → multi-modal alignment baseline<br>• Meta MAE → masking strategy 참고 |
| 선행사업 모델 활용/개선 | 10% | • BrainIAC (Stanford) 아키텍처 참고 후 multi-modal 확장<br>• 기존 fMRI-only → fMRI+dMRI+EEG 통합으로 개선 |
...
**형식**: 연도별 테이블 + 사업 기간 선택 (3년/4년/5년 중 택1)
...
| **기타 AI 관련 비용** (인건비, 임상시험, platform) | 550 | 530 | 950 | 1,515 | 1,388 | 4,933 |
...
### 즉시 작성 (Week 1)
1. ✅ 섹션 1 - 사업 내용 (가장 중요, 심사위원 첫 인상 결정)
   - 특히 "사업 목적", "사업 주요내용", "정부 지원 필요성" 집중
2. ✅ 섹션 4 - 예산 규모 (검증 가능성, 실현 가능성 판단 기준)

### 단기 작성 (Week 2)
3. ✅ 섹션 2.1-2.2 - 데이터 및 AI 개발 (기술적 타당성 입증)
4. ✅ 섹션 3 - 연도별 기술개발 사항 (구체적 실행 계획)

### 최종 검토 (Week 3)
5. ✅ 섹션 2.3 - AI 활용 (파급효과 강조)
6. ✅ 전체 일관성 검토 및 수치 검증
7. ✅ 문법/오타 교정
8. ✅ 한글 양식 변환 (.hwpx 파일 생성)
...
### DO (반드시 하기)
1. **정량화**: 모든 목표를 숫자로 표현 (데이터 규모, 성능 지표, 예산)
2. **구체화**: 모호한 용어 금지 ("개별 양식" → "Vision Transformer")
3. **검증 가능성**: 주장에 대한 근거 제시 (벤치마크, 선행연구 인용)
4. **임팩트 명시**: 과학/경제/사회 3가지 측면 모두 다룸
5. **차별성 강조**: 국제 프로젝트 대비 한국의 우위 명확히

### DON'T (피해야 할 것)
1. ❌ 문법 오류 ("활용하기 될 수 있음")
2. ❌ 미완성 표 (예산 백만원 빈 칸)
3. ❌ 과장된 주장 (근거 없는 "세계 최초", "혁명적")
4. ❌ 모호한 표현 ("다양한", "여러", "많은")
5. ❌ 기술 과잉 (심사위원이 이해 못할 수식/전문용어)

---

## 🔄 Enhanced Proposal 활용 전략

### 직접 활용 가능 (70%)
- 기술적 세부사항 (아키텍처, 학습 방법, 컴퓨팅)
- 정량적 목표 (성능 지표, 예산 배분)
- 국제 벤치마크 비교
- 임상시험 계획

### 압축 필요 (20%)
- Executive Summary → 사업 목적 (500 words → 200 words)
- Technical Approach → 사업 주요내용 (6 pages → 1 page)
- Milestone Table → 연도별 기술개발 (분기별 → 연간)

### 신규 작성 필요 (10%)
- 한국 정부 R&D 특화 내용:
  • 정부 지원 필요성 (왜 정부가, 왜 지금, 왜 한국인가?)
  • AI 기술 발전/보급 기여도 (정책 관점)
  • 파급효과 (국내 산업 중심)
...
1. **섹션 1 작성**: 사업 내용 테이블 완성 (Enhanced Proposal 기반)
2. **섹션 4 작성**: 예산 규모 세부 항목 (총 ₩101.33억 분해)
3. **섹션 2-3 작성**: AI 특성 및 기술개발 사항
4. **최종 문서 생성**: 한글 양식 (.hwpx) 파일 생성
